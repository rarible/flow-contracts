...
var (
	...
	RaribleOpenBidAddressPlaceholder = regexp.MustCompile(`"[^"\s].*/RaribleOpenBid.cdc"`)
)
...
type Contracts struct {
	...
	RaribleOpenBidAddress flow.Address
	RaribleOpenBidSigner  crypto.Signer
}
...
func RaribleOpenBidDeployContracts(t *testing.T, b *emulator.Blockchain) Contracts {
	accountKeys := test.AccountKeyGenerator()

	nftAddress, exampleNFTAddress, exampleNFTSigner := deployNFTContracts(t, b)

	RaribleOpenBidAccountKey, RaribleOpenBidSigner := accountKeys.NewWithSigner()
	RaribleOpenBidCode := loadRaribleOpenBid(ftAddress, nftAddress)

	RaribleOpenBidAddress, err := b.CreateAccount(
		[]*flow.AccountKey{RaribleOpenBidAccountKey},
		nil,
	)
	require.NoError(t, err)

	fundAccount(t, b, RaribleOpenBidAddress, defaultAccountFunding)

	tx := sdktemplates.AddAccountContract(
		RaribleOpenBidAddress,
		sdktemplates.Contract{
			Name:   "RaribleOpenBid",
			Source: string(RaribleOpenBidCode),
		},
	)
	...
	signAndSubmit(
		t, b, tx,
		[]flow.Address{b.ServiceKey().Address, RaribleOpenBidAddress},
...
func setupRaribleOpenBid(
	...
	tx := flow.NewTransaction().
		SetScript(RaribleOpenBidGenerateSetupAccountScript(contracts)).
		SetGasLimit(100).
		SetProposalKey(b.ServiceKey().Address, b.ServiceKey().Index, b.ServiceKey().SequenceNumber).
		SetPayer(b.ServiceKey().Address).
...
func setupAccount(
	...
	setupRaribleOpenBid(t, b, address, signer, contracts)
